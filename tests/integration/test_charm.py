...
async def test_build_and_deploy(ops_test, charm_under_test):
    ...
    await ops_test.model.deploy(charm_under_test, resources=resources, application_name="proxy", series="focal")
...
async def test_charm_goes_into_active_state_after_alertmanager_ip_provided(ops_test):
    ...
    async with IPAddressWorkaround(ops_test):
        await ops_test.model.deploy("ch:alertmanager-k8s", application_name="am", channel="edge", series="focal")
...
async def test_karma_charm_goes_into_active_state_after_related_to_proxy(ops_test):
    ...
    # deploy karma
    await ops_test.model.deploy("ch:karma-k8s", application_name="karma", channel="edge", series="focal")
